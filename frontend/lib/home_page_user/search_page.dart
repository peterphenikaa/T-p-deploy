...
class _SearchPageState extends State<SearchPage> {
  ...
  Future<void> _searchFoods(String query) async {
    ...
    try {
      ...
      final url = Uri.parse('$baseUrl/api/foods?search=${Uri.encodeQueryComponent(query.trim())}');
            ...
            children: [
              Row(
                children: [
                  ...
                  Text("Tìm kiếm", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 19)),
                  Spacer(),
              ...
              _searchText.isEmpty
                ...
                child: ListView(
                  children: [
                    Text("Recent Keywords", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                    SizedBox(height: 9),
                    Wrap(
                      spacing: 10,
                      children: recentKeywords.isEmpty
                          ? [Text("Chưa có từ khóa tìm gần đây")]
                          : recentKeywords.map((key) => ActionChip(
                        label: Text(key),
                        onPressed: () {
                    ...
                    SizedBox(height: 20),
                    Text("Suggested Restaurants", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                    SizedBox(height: 7),
                    ...suggestedRestaurants.map((res) => ListTile(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => RestaurantDetailPage(
                              restaurant: {
                                ...
                                'rating': res['rating'],
                                'deliveryTime': 20,
                                'description': 'Nhà hàng ${res['name']} với hương vị đặc biệt và chất lượng cao. Chúng tôi cam kết mang đến những món ăn tươi ngon nhất cho khách hàng.',
                              },
                            ),
                          ),
                        );
                      ...
                      leading: ClipRRect(
                        ...
                        child: Image.asset(res['image'], width: 42, height: 42, fit: BoxFit.cover),
                      ),
                      title: Text(res['name'], style: TextStyle(fontWeight: FontWeight.w500)),
                      subtitle: Row(
                        children: [
                          Icon(Icons.star, size: 16, color: Colors.orange),
                          SizedBox(width: 2),
                          Text(res['rating'].toString(), style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                        ],
                      ),
                    ...
                    SizedBox(height: 13),
                    Text("Popular Fast Food", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                    SizedBox(height: 8),
                    SizedBox(
                      height: 75,
                      child: ListView(
                        scrollDirection: Axis.horizontal,
                        children: popularFoods.map((food) => GestureDetector(
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ProductDetailPage(
                                  product: {
                                    ...
                                    'rating': 4.5,
                                    'category': 'Fast Food',
                                    'description': 'Món ăn phổ biến từ ${food['restaurant']}',
